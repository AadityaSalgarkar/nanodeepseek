...
class ModelArgs:
    ...
    Attributes:
        ...
        dtype (Literal["bf16"]): Data type for computations.
        ...
        sliding_window_size (int): Size of the sliding window for sparse attention.
        compress_block_size (int): Size of the compress block for sparse attention.
        selection_block_size (int): Size of the selection block for sparse attention.
        num_selected_blocks (int): Number of selected blocks for sparse attention.
    """
...
def linear(
    ...
    Ensures consistent bfloat16 computation.
...
class DyTNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        """
        Forward pass for DyTNorm.
        returns gamma * tanh(alpha * x ) + beta
        ...
        """
        normalized = F.tanh(self.alpha * x)
...
class Block(nn.Module):
    """
    Transformer block combining sparse attention and feed-forward layers.

    Attributes:
        attn (nn.Module): Sparse attention layer.
...
class DynamicTanh(nn.Module):
    def __init__(
        ...
        self.actual_tanh = use_actual_tanh
    ...
    def forward(self, x):
        if self.actual_tanh:
            ...
            """
            use this approximation of tanh(z)
            z(10+z^2)(60+z^2)/(600+270z^2+11z^4+z^6/24)
            """
            z = self.alpha * x
            z2 = z * z
            numerator = z * (10 + z2) * (60 + z2)
            denominator = 600 + 270 * z2 + 11 * z2 * z2 + (z2 * z2 * z2) / 24 + self.eps
...
def convert_rmsn_to_dyt(module):
    module_output = module
    if isinstance(module, nn.RMSNorm):
        module_output = DynamicTanh(module.normalized_shape, False)
    for name, child in module.named_children():
        module_output.add_module(name, convert_rmsn_to_dyt(child))
    del module
